...
Condorcet requires independent voters. In practice, independence is threatened at three layers:

1. **Model family.** All N researchers run on the same model (Claude) via Claude Code's subscription infrastructure. They share architecture, training data, and systematic biases. The theoretical fix — diverse model families (Claude, GPT, Gemini) — is beyond scope. This is an accepted trade-off: fixed-cost subscriptions make ensembling economically viable, but constrain all researchers to the same training distribution.

2. **Retrieval layer.** Claude Code's built-in WebFetch tool summarizes web content through a small model, likely cached. All researchers hitting the same URL receive the same lossy summary — identical input silently destroying independence at the point where diversity matters most. The skill instructs researchers to use `curl` instead, returning raw content each researcher interprets independently.

3. **Prompt layer.** All researchers receive identical prompts. This is intentional — prompt variation would make output differences reflect prompt differences, not genuine diversity of findings. Independence comes from each researcher choosing their own path through the search space, not from different instructions.

Empirical work confirms the model-family problem: studies on LLM ensemble sentiment analysis ([arXiv:2409.00094](https://arxiv.org/abs/2409.00094)) found only marginal improvements from majority voting because models make correlated errors. "Consensus is Not Verification" ([arXiv:2603.06612](https://arxiv.org/abs/2603.06612)) showed that no aggregation strategy consistently beats single-sample baselines when agents share systematic biases.

This means p_hat estimated from agreement is an **upper bound**, not a direct measurement of accuracy. When all five researchers agree on a wrong fact (because it's wrong in their shared training data), agreement is high but accuracy is zero. The fact-checking phase compensates — it is the only step that introduces signal from outside the model, via web search against authoritative sources. The correlation and fact-checking phases are not optional polish but the mechanism that converts unreliable consensus into verified accuracy.

The quality metrics are transparent about this gap: q_final decomposes into ensemble_acc (theoretical, assumes independence) × calibration (empirical, measures how often the fact-checking phase actually corrected the ensemble). The calibration term is where reality corrects theory.
